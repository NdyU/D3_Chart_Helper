...
var barYGraph = function(data, margin, cWidth, cHeight, g, container) {
  ...
  .attr("x", function(d,i) {return rangeScale(i);})
  .attr("y", function(d,i) {return (containerHeight+margin.top)-dataScale(d);})
  .attr("width", width)
 ...
 console.log(rectAttributes);
  ...
  var yScale  = d3.scaleLinear()
  .domain([min, max])
  .range([containerHeight+margin.top, margin.top]);
...
var barXGraph = function(data, margin, cWidth, cHeight, g, container, landscape) {
  ...
  var dataScale = d3.scaleLinear()
    .domain([min, max])
    .range([10, containerWidth])
    

  var rangeScale = d3.scaleLinear()
    .domain([0, dataSize-1])
    .range([containerHeight-width, margin.top]);
  ...
  var rectAttributes = rects
  .attr("x", margin.left)
  .attr("y", function(d,i) {return rangeScale(i);})
  .attr("width", function(d,i) {return dataScale(d);})
  .attr("height", width)
  .attr("id", function(d,i) {return i;})
  .style("fill", "lightblue");
